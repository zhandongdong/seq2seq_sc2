...
def trainIters_sc2(enemyUnits, encoder, decoder, n_iters, print_every=1000, plot_every=1000,
    ...
    for iter in range(1, n_iters + 1):
        ...
        if iter % print_every == 0:
            ...
            torch.save(encoder1.state_dict(), encode_model_path + "_" + str(iter) + ".pkl")
            torch.save(decoder1.state_dict(), decode_model_path + "_" + str(iter) + ".pkl")
...
if not os.path.exists(encode_model_path):
    ...
    ######################################################################
    print("save model:")
    torch.save(encoder1.state_dict(), encode_model_path)
    torch.save(decoder1.state_dict(), decode_model_path)
else:
    print("already exist, load model:")
    encoder1 = encoder1.load_state_dict(torch.load(encode_model_path))
...
# print(cents)
decoder1 = decoder1.load_state_dict(torch.load(decode_model_path))
